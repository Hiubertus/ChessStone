...
import {BoardConfig, PlayerConfig} from "@/types/BoardConfig.ts";
...
export const standardBoardConfig = (): BoardConfig => {
    ...
    const generateId = (type: ChessPiece['type'], color: ChessPiece['color'], index: number) => `${color}_${type}_${index}`;
